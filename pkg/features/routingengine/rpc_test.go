...
func TestParseOutputMultiRE(t *testing.T) {
	body := `<multi-routing-engine-results>
...
<cli>
	...
	assert.Equal(t, "node0", rpc.RoutingEngines[0].Name, "re-name")

	// test first route engine
	assert.Equal(t, "OK", rpc.RoutingEngines[0].Information.RouteEngines[0].Status, "status")

	assert.Equal(t, float64(35), rpc.RoutingEngines[0].Information.RouteEngines[0].CPUTemperature.Value, "cpu-temperature")

	assert.Equal(t, float64(1905), rpc.RoutingEngines[0].Information.RouteEngines[0].MemorySystemTotal, "memory-system-total")

	assert.Equal(t, float64(19), rpc.RoutingEngines[0].Information.RouteEngines[0].CPUUser1, "cpu-user1")
	// test routing engine 1
	assert.Equal(t, "node1", rpc.RoutingEngines[1].Name, "re-name")

	assert.Equal(t, "OK", rpc.RoutingEngines[1].Information.RouteEngines[0].Status, "status")

	assert.Equal(t, float64(36), rpc.RoutingEngines[1].Information.RouteEngines[0].CPUTemperature.Value, "cpu-temperature")
}

// Test no multi routing engine
func TestParseOutputNoMultiRE(t *testing.T) {
	body := `<route-engine-information xmlns="http://xml.juniper.net/junos/ZZZ/junos-chassis">
    <route-engine>
        <slot>0</slot>
...
<cli>
	...
	assert.Equal(t, "N/A", rpc.RoutingEngines[0].Name, "re-name")

	// test first route engine
	assert.Equal(t, "0", rpc.RoutingEngines[0].Information.RouteEngines[0].Slot, "slot")

	assert.Equal(t, float64(31), rpc.RoutingEngines[0].Information.RouteEngines[0].CPUTemperature.Value, "cpu-temperature")

	// test second route engine
	assert.Equal(t, "1", rpc.RoutingEngines[0].Information.RouteEngines[1].Slot, "slot")

	assert.Equal(t, uint64(3149860), rpc.RoutingEngines[0].Information.RouteEngines[1].UpTime.Seconds, "up-time")
